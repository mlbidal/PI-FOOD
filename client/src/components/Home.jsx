...
/*export default function Home() {
  const dispatch = useDispatch(); //para usar la const para ir despachando mis acciones
  const allRecipes = useSelector((state) => state.recipes); // es lo mismo que hacer el maps con el props en clases
  const diets = useSelector((state) => state.diets);
  //PAGINADO: me defino varios estados locales
  const [currentPage, setCurrentPage] = useState(1); // Guardo en un estado local la pagina actual, comienza en 1 el use state xq siempre arranco en la primera pagina
  const [recipesPerPage, setRecipesPerPage] = useState(9); // guardo cuantos personajes quiero por pagina
  ...
  function handleClick(e) { //boton que me trae todas las recetas de nuevo
    e.preventDefault();
    dispatch(getRecipes());
  }
